 ...
 * 
 * VM Instructions Set
 * - EIT    rs1         : Execute a reaction (rs1) If Triggered.
 ...
 * - ADV    rs1,    rs2 : ADVance the logical time of a reactor (rs1) by a specified amount (rs2).
 * - JMP    rs1         : JuMP to a location.
 * - INC    rs1,    rs2 : INCrement a counter (rs1) by an amount (rs2).
 * - SAC                : (Sync-And-Clear) synchronize all workers until all execute SYN and let the last idle worker reset all counters to 0.
...
// FIXME: Race condition between CLR and INC.
// Need to implement a SYNC instruction to synchronize
// threads to the hyperperiod barrier before jumping
// to the next iteration together.
...
const inst_t schedule_0[] = {
    ...
    // {.op=INC,   .rs1=0,     .rs2=1},        // INC counter 0 by 1
    // {.op=WU,    .rs1=0,     .rs2=2},        // WU  counter 0 reaches 2
    // {.op=EXE,   .rs1=3,     .rs2=-1},       // EIT sink.1
    // {.op=INC,   .rs1=1,     .rs2=1},        // INC counter 1 by 1
    // {.op=WU,    .rs1=1,     .rs2=2},        // WU  counter 1 reaches 2
    // {.op=EXE,   .rs1=4,     .rs2=-1},       // EIT sink.2
    // {.op=ADV,   .rs1=2,     .rs2=5000000},  // ADV sink, 5000000
    // {.op=EXE,   .rs1=2,     .rs2=-1},       // EXE sink.0
    // {.op=ADV,   .rs1=2,     .rs2=5000000},  // ADV sink, 5000000
    // {.op=SAC,   .rs1=-1,    .rs2=-1},       // Sync all workers And Clear counters
    // {.op=STP,   .rs1=-1,     .rs2=-1},      // JMP to line 0
};
